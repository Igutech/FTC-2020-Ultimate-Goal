 ...
 * 5. Use the right joystick on gamepad 1 to turn the bot.
 * 6. Spin the bot 10 times. Make sure to keep track of these turns.
...
public class TrackingWheelLateralDistanceTuner extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        telemetry.addLine("Press A to stop the routine.");
        ...
        while (!isStopRequested() && !tuningFinished) {
            Pose2d vel = new Pose2d(0, 0, -gamepad1.right_stick_x * OMEGA_WEIGHT);
            ...
            headingAccumulator += Angle.norm(deltaHeading);
            lastHeading = heading;

            telemetry.clearAll();
            telemetry.addLine("Heading (deg): " + Math.toDegrees(headingAccumulator));
            telemetry.addLine();
            telemetry.addLine("Press A to conclude routine");
            telemetry.update();
        ...
        telemetry.clearAll();
        telemetry.addLine("Localizer's accumulated heading: " + headingAccumulator);
        ...
        System.out.println("LD: "+(headingAccumulator / (NUM_TURNS * Math.PI * 2)) * StandardTrackingWheelLocalizer.LATERAL_DISTANCE);
        System.out.println("LD 2: "+((NUM_TURNS * Math.PI * 2) / headingAccumulator) * StandardTrackingWheelLocalizer.LATERAL_DISTANCE);
        telemetry.update();
    }
}